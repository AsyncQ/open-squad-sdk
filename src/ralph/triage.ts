...
// ─── LLM-assisted triage with escalation ─────────────────────────────────────

export interface LlmTriageConfig {
  /** Model for the first LLM triage attempt */
  defaultModel: string;
  /** Model for the second attempt when the first is unresolved or low-confidence */
  escalationModel?: string;
  /** OpenAI-compatible base URL (OpenRouter or local LLM) */
  baseUrl: string;
  /** API key — empty string for local LLMs */
  apiKey?: string;
}

export interface LlmTriageResult {
  decision: TriageDecision | null;
  /** true if the escalation model was needed to reach this decision */
  escalationUsed: boolean;
  /** true if both models failed — caller should write to developer inbox */
  needsDeveloper: boolean;
}

/**
 * Triage with LLM escalation.
 *
 * Flow:
 *   1. Rule-based triage (no LLM, zero cost)
 *   2. If null or low-confidence → LLM call with defaultModel
 *   3. If still unresolved → LLM call with escalationModel
 *   4. If still unresolved → needsDeveloper = true
 */
export async function triageIssueWithLlmEscalation(
  issue: TriageIssue,
  rules: RoutingRule[],
  modules: ModuleOwnership[],
  roster: TeamMember[],
  llmConfig: LlmTriageConfig,
): Promise<LlmTriageResult> {
  // Step 1 — rule-based (free)
  const ruleDecision = triageIssue(issue, rules, modules, roster);
  if (ruleDecision && ruleDecision.confidence !== 'low') {
    return { decision: ruleDecision, escalationUsed: false, needsDeveloper: false };
  }

  const rosterSummary = roster.map(m => `${m.name} (${m.role})`).join(', ');
  const prompt = buildTriagePrompt(issue, rosterSummary, ruleDecision);

  // Step 2 — LLM with defaultModel
  const firstAttempt = await callLlmForTriage(prompt, llmConfig.defaultModel, llmConfig, roster);
  if (firstAttempt) {
    return { decision: firstAttempt, escalationUsed: false, needsDeveloper: false };
  }

  // Step 3 — LLM with escalationModel (if configured)
  if (llmConfig.escalationModel && llmConfig.escalationModel !== llmConfig.defaultModel) {
    const secondAttempt = await callLlmForTriage(
      prompt,
      llmConfig.escalationModel,
      llmConfig,
      roster,
    );
    if (secondAttempt) {
      return { decision: secondAttempt, escalationUsed: true, needsDeveloper: false };
    }
  }

  // Step 4 — both models failed, escalate to developer
  return { decision: ruleDecision, escalationUsed: true, needsDeveloper: true };
}

function buildTriagePrompt(
  issue: TriageIssue,
  rosterSummary: string,
  ruleDecision: TriageDecision | null,
): string {
  const ruleHint = ruleDecision
    ? `Rule-based triage suggested "${ruleDecision.agent.name}" with low confidence: ${ruleDecision.reason}.`
    : 'Rule-based triage found no match.';

  return `You are a software project triage coordinator.
...
Which team member should handle this issue? Reply with ONLY a JSON object:
{"agentName": "<name>", "reason": "<one sentence>", "confidence": "high|medium|low"}`;
}
...
async function callLlmForTriage(
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { agentName?: string; reason?: string; confidence?: string };

    if (!parsed.agentName) return null;

    const agent = roster.find(m => m.name.toLowerCase() === parsed.agentName!.toLowerCase());
    ...
    return {
      agent,
      reason: parsed.reason ?? `LLM assigned to ${agent.name}`,
      source: 'routing-rule',
      confidence,
    };
  } catch {
