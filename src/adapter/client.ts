 ...
 *
 * Provider-agnostic client. Selects a backend at construction time based on
 * SquadClientOptions.provider — or falls back to CopilotBackend when no
 * provider is supplied (preserves full backward compatibility).
 *
 * Backends:
 *   - CopilotBackend          → default, requires @github/copilot-sdk
 *   - OpenAICompatibleBackend → OpenRouter or any local LLM (LM Studio, Ollama…)
 *   - AnthropicBackend        → direct Anthropic API, requires @anthropic-ai/sdk
 *
...
import { CopilotBackend } from './backends/copilot.js';
...
export interface SquadClientOptions {
  ...
  /**
   * GitHub token for authentication (Copilot backend only).
   * If not provided, uses logged-in user credentials.
   */
  githubToken?: string;

  /**
   * Use logged-in user credentials for authentication (Copilot backend only).
   * @default true (false if githubToken is provided)
   */
  useLoggedInUser?: boolean;

  /**
   * Provider configuration — selects the inference backend.
   *
   * Omit for default Copilot behaviour (backward compatible).
   *
   * OpenRouter:
   *   { type: 'openai', baseUrl: 'https://openrouter.ai/api/v1', apiKey: '...' }
   *
   * Local LLM (LM Studio / Ollama):
   *   { type: 'openai', baseUrl: 'http://localhost:1234/v1' }
   *
   * Direct Anthropic:
...
export class SquadClient {
  ...
  constructor(options: SquadClientOptions = {}) {
    ...
    this.client = SquadClient.createBackend(options);
  }

  /**
   * Selects and instantiates the correct backend from provider config.
   *
   * Priority:
   *   1. provider.type === 'anthropic'  → AnthropicBackend
   *   2. provider.type === 'openai' (or any baseUrl present) → OpenAICompatibleBackend
   *   3. No provider                    → CopilotBackend  (default, backward-compatible)
   */
  private static createBackend(options: SquadClientOptions): ISquadBackend {
    ...
    if (p?.baseUrl || p?.type === 'openai' || p?.type === 'azure') {
      return new OpenAICompatibleBackend({
        baseUrl: p.baseUrl,
        apiKey: p.apiKey ?? p.bearerToken ?? '',
        defaultHeaders: p.type === 'openai' && p.baseUrl?.includes('openrouter')
          ? { 'HTTP-Referer': 'https://github.com/AsyncQ/open-squad-sdk' }
          : undefined,
      });
    }

    // Default: Copilot (requires @github/copilot-sdk)
